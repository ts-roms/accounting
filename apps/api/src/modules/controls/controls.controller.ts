...
export class ControlsController {
  ...
  @RequirePermissions(P['controls.view'])
  @ApiOperation({
    summary: 'Suspense / clearing account monitor: balance, age, status per account',
  })
  suspenseMonitor(@CurrentUser() user: AuthenticatedUser, @Query() query: ControlsQueryDto) {
    return this.suspense.monitor(user.companyId!, query.asOf ?? today());
  }
}
