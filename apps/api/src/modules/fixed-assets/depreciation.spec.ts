...
describe('depreciation', () => {
  it('straight line spreads cost - salvage evenly and lands exactly on salvage', () => {
    const s = schedule(
      { ...base, cost: '100000.0000', salvageValue: '1000.0000', usefulLifeMonths: 7 },
      PHP,
    );
  ...
  it('impairment shortens the remaining schedule rather than the monthly rate', () => {
    ...
    const amount = monthlyDepreciation(
      { ...base, accumulatedDepreciation: '60000.0000', depreciatedMonths: 12 },
      PHP,
    );
  ...
  it('fully depreciated assets stop; disposal gain / loss is proceeds less book value', () => {
    expect(
      monthlyDepreciation(
        { ...base, accumulatedDepreciation: '120000.0000', depreciatedMonths: 36 },
        PHP,
      ).isZero(),
    ).toBe(true);
    expect(
      disposalGainLoss(
        { cost: '120000', accumulatedDepreciation: '80000' },
        '50000',
        PHP,
      ).toString(),
    ).toBe('10000.0000');
    expect(
      disposalGainLoss({ cost: '120000', accumulatedDepreciation: '80000' }, '0', PHP).toString(),
    ).toBe('-40000.0000');
  });
});
