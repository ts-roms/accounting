...
describe('Budgeting, dimensions, tax & expense claims (e2e)', () => {
  ...
  const as = (req: request.Test, who: Cookies = admin) => req.set('Cookie', who).set('x-company-id', companyId).set(CSRF);
  ...
  const trialBalanced = async () => {
    const tb = await as(http().get('/api/v1/reports/trial-balance?from=2026-01-01&to=2026-12-31')).expect(200);
  ...
  beforeAll(async () => {
    ...
    await runSeed(DB_URL, { adminEmail: ADMIN.email, adminPassword: ADMIN.password, log: () => undefined });
  ...
  it('dimensions are typed: a project column cannot point at a department', async () => {
    ...
    const created = await as(http().post('/api/v1/dimensions')).send({ dimensionType: 'DEPARTMENT', code: 'RND', name: 'Research' }).expect(201);
    expect(created.body.usageCount).toBe(0);
    const dup = await as(http().post('/api/v1/dimensions')).send({ dimensionType: 'DEPARTMENT', code: 'RND', name: 'Again' }).expect(409);
  ...
  it('journal lines carry dimensions and the ledger / trial balance filter by them', async () => {
    const je = await as(http().post('/api/v1/journal-entries'))
      .send({
        ...
        lines: [
          { accountId: acc['6400'], debit: '3000', departmentId: dim['SALES'], costCenterId: dim['CC-HQ'], projectId: dim['PRJ-ERP'] },
    ...
    const byDept = await as(http().get(`/api/v1/general-ledger?accountId=${acc['6400']}&from=2026-09-01&to=2026-09-30&departmentId=${dim['SALES']}`)).expect(200);
    expect(byDept.body.lines).toHaveLength(1);
    expect(byDept.body.lines[0].debit).toBe('3000.0000');
    const other = await as(http().get(`/api/v1/general-ledger?accountId=${acc['6400']}&from=2026-09-01&to=2026-09-30&departmentId=${dim['OPS']}`)).expect(200);
    expect(other.body.lines).toHaveLength(0);
    const tb = await as(http().get(`/api/v1/reports/trial-balance?from=2026-09-01&to=2026-09-30&projectId=${dim['PRJ-ERP']}`)).expect(200);
  ...
  it('tax codes carry effective-dated rates; overlapping windows are rejected', async () => {
    ...
    const noAccount = await as(http().post('/api/v1/tax/codes')).send({ code: 'BAD', name: 'x', kind: 'SALES_TAX', appliesTo: 'BOTH', reportingCategory: 'TAXABLE', salesAccountId: acc['2130'], rates: [{ ratePercent: '1', effectiveFrom: '2020-01-01' }] }).expect(400);
  ...
  it('invoice with VAT and creditable withholding: total = subtotal + tax - withholding, posted per tax account', async () => {
    const inv = await as(http().post('/api/v1/invoices'))
      .send({
        ...
        lines: [
          { description: 'Consulting', unitPrice: '10000', accountId: acc['4200'], taxCodeId: tax['VAT12'], withholdingTaxCodeId: tax['EWT2'], departmentId: dim['SALES'] },
          { description: 'Zero-rated export', unitPrice: '4000', accountId: acc['4100'], taxCodeId: tax['VAT0'] },
        ],
      })
      .expect(201);
    ...
    const wrongSide = await as(http().post('/api/v1/invoices'))
      .send({ customerId: party['CUST-001'], documentDate: '2026-09-05', lines: [{ description: 'x', unitPrice: '1', accountId: acc['4100'], taxCodeId: tax['EWT1'] }] })
      .expect(422);
    ...
    const txs = await as(http().get(`/api/v1/tax/transactions?from=2026-09-01&to=2026-09-30`)).expect(200);
    expect(txs.body.items).toHaveLength(3); // VAT12, VAT0 (zero-rated base is still reported), EWT2
    const summary = await as(http().get('/api/v1/tax/reports/summary?from=2026-09-01&to=2026-09-30')).expect(200);
  ...
  it('bill with input VAT and expanded withholding; the tax summary nets output against input', async () => {
    const bill = await as(http().post('/api/v1/bills'))
      .send({
        ...
        lines: [{ description: 'Cleaning services', unitPrice: '5000', accountId: acc['6400'], taxCodeId: tax['VAT12'], withholdingTaxCodeId: tax['EWT1'], costCenterId: dim['CC-HQ'] }],
      })
      .expect(201);
    ...
    const wht = await as(http().get('/api/v1/tax/reports/withholding?from=2026-09-01&to=2026-09-30&side=PURCHASES')).expect(200);
  ...
  it('voiding a taxed invoice reverses its tax transactions', async () => {
    await as(http().post(`/api/v1/invoices/${invoiceId}/void`), finance).send({ reason: 'Re-issue' }).expect(201);
    const summary = await as(http().get('/api/v1/tax/reports/summary?from=2026-09-01&to=2026-09-30')).expect(200);
    expect(summary.body.totals.outputTax).toBe('0.0000');
    expect(summary.body.totals.withholdingReceivable).toBe('0.0000');
    expect(summary.body.totals.inputTax).toBe('600.0000');
    const txs = await as(http().get(`/api/v1/tax/transactions?from=2026-09-01&to=2026-09-30&side=SALES`)).expect(200);
    expect(txs.body.items.filter((t: { reversalOfId: string | null }) => t.reversalOfId)).toHaveLength(3);
    await trialBalanced();
  });
  ...
  it('the seeded budget compares against posted actuals per account and period', async () => {
    ...
    const byDept = await as(http().get(`/api/v1/budgets/${opex.id}/variance?departmentId=${dim['SALES']}`)).expect(200);
  ...
  it('budget versions: draft lines replace, approval locks and supersedes', async () => {
    ...
    const created = await as(http().post('/api/v1/budgets')).send({ fiscalYearId: year.id, code: 'CAPEX-2026', name: 'Capital plan' }).expect(201);
    ...
    const lines = await as(http().put(`/api/v1/budgets/${created.body.id}/versions/${v1.id}/lines`))
      .send({ lines: [{ accountId: acc['6500'], fiscalPeriodId: jan.id, amount: '12000', departmentId: dim['OPS'] }, { accountId: acc['6500'], fiscalPeriodId: jan.id, amount: '3000', departmentId: dim['ADMIN'] }] })
      .expect(200);
    expect(lines.body.lines).toHaveLength(2);
    expect(lines.body.total).toBe('15000.0000');
    const header = await as(http().put(`/api/v1/budgets/${created.body.id}/versions/${v1.id}/lines`)).send({ lines: [{ accountId: acc['6000'], fiscalPeriodId: jan.id, amount: '1' }] }).expect(422);
    expect(header.body.message).toMatch(/header account/);
    await as(http().post(`/api/v1/budgets/${created.body.id}/versions/${v1.id}/approve`), finance).expect(201);
    const locked = await as(http().put(`/api/v1/budgets/${created.body.id}/versions/${v1.id}/lines`)).send({ lines: [] }).expect(422);
    expect(locked.body.code).toBe('DOCUMENT_INVALID_STATE');
    const v2 = await as(http().post(`/api/v1/budgets/${created.body.id}/versions`)).send({ name: 'Revised', copyFromVersionId: v1.id }).expect(201);
    expect(v2.body.versionNumber).toBe(2);
    expect(v2.body.lines).toHaveLength(2);
    await as(http().post(`/api/v1/budgets/${created.body.id}/versions/${v2.body.id}/approve`), finance).expect(201);
    const detail = await as(http().get(`/api/v1/budgets/${created.body.id}`)).expect(200);
    expect(detail.body.status).toBe('ACTIVE');
    expect(detail.body.versions.find((v: { id: string }) => v.id === v1.id).status).toBe('SUPERSEDED');
    expect(detail.body.approvedVersionId).toBe(v2.body.id);
    const variance = await as(http().get(`/api/v1/budgets/${created.body.id}/variance?departmentId=${dim['OPS']}`)).expect(200);
    expect(variance.body.rows.find((r: { code: string }) => r.code === '6500').cells[0].budget).toBe('12000.0000');
  });
  ...
  it('expense claim: tax-inclusive lines, submit, own-approval blocked, approve, post and reimburse', async () => {
    const claim = await as(http().post('/api/v1/expense-claims'))
      .send({
        ...
        lines: [
          { expenseDate: '2026-09-07', description: 'Taxi', accountId: acc['6400'], amount: '1120', taxCodeId: tax['VAT12'], departmentId: dim['SALES'] },
          { expenseDate: '2026-09-07', description: 'Parking (no receipt)', accountId: acc['6400'], amount: '200' },
        ],
      })
      .expect(201);
    ...
    const late = await as(http().post('/api/v1/expense-claims')).send({ claimDate: '2026-09-08', purpose: 'x', lines: [{ expenseDate: '2026-09-09', description: 'x', accountId: acc['6400'], amount: '1' }] }).expect(422);
    expect(late.body.code).toBe('VALIDATION_FAILED');

    const tooEarly = await as(http().post(`/api/v1/expense-claims/${claim.body.id}/approve`), finance).expect(422);
    ...
    const own = await as(http().post(`/api/v1/expense-claims/${claim.body.id}/approve`)).expect(422);
    expect(own.body.code).toBe('SOD_VIOLATION');
    const approved = await as(http().post(`/api/v1/expense-claims/${claim.body.id}/approve`), finance).expect(201);
    expect(approved.body.status).toBe('APPROVED');

    const posted = await as(http().post(`/api/v1/expense-claims/${claim.body.id}/post`), finance).expect(201);
    const je = posted.body.journalEntryId;
    const expense = await journalLine(je, '6400');
    expect(expense.map((l: { debit: string }) => l.debit).sort()).toEqual(['1000.0000', '200.0000']);
    expect((await journalLine(je, '1450'))[0].debit).toBe('120.0000');
    expect((await journalLine(je, '2170'))[0].credit).toBe('1320.0000');
    const editLocked = await as(http().patch(`/api/v1/expense-claims/${claim.body.id}`)).send({ purpose: 'x' }).expect(422);
    ...
    const paid = await as(http().post(`/api/v1/expense-claims/${claim.body.id}/pay`), finance).send({ bankAccountId: bdo.id, paymentDate: '2026-09-10', reference: 'REIMB-1' }).expect(201);
    expect(paid.body.status).toBe('PAID');
    expect((await journalLine(paid.body.paymentJournalEntryId, '2170'))[0].debit).toBe('1320.0000');
    expect((await journalLine(paid.body.paymentJournalEntryId, '1130'))[0].credit).toBe('1320.0000');
    const summary = await as(http().get('/api/v1/tax/reports/summary?from=2026-09-01&to=2026-09-30')).expect(200);
