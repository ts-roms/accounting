...
describe('Enterprise controls (e2e)', () => {
  ...
  it('suspense monitor: aged balances require investigation, block the close and show on the integrity report until cleared', async () => {
    const clean = await as(http().get('/api/v1/accounting/suspense')).expect(200);
    expect(clean.body.accounts.map((a: { code: string }) => a.code)).toEqual(
      expect.arrayContaining(['1590', '1900', '2160']),
    );
    ...
    const parked = await postJournal(
      ...
      [
        ...
        { code: '1900', credit: '5000' },
      ],
      'Unidentified receipt',
    );
    const aged = await as(http().get('/api/v1/accounting/suspense')).expect(200);
    const suspense = aged.body.accounts.find((a: { code: string }) => a.code === '1900');
    ...
    await as(http().patch('/api/v1/accounting-policies'))
      ...
      .expect(200);
    const relaxed = await as(http().get('/api/v1/accounting/suspense')).expect(200);
    expect(relaxed.body.accounts.find((a: { code: string }) => a.code === '1900').status).toBe(
      'WITHIN_POLICY',
    );
    await as(http().patch('/api/v1/accounting-policies'))
    ...
    await postJournal(
      daysAgo(1),
      [
        { code: '1900', debit: '5000' },
        { code: '1120', credit: '5000' },
      ],
      'Receipt identified',
    );
    const cleared = await as(http().get('/api/v1/accounting/suspense')).expect(200);
