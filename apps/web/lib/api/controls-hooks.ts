...
import type { ControlDashboard, FieldChange, SodUserConflict, SuspenseMonitor } from './types';
...
export const useSuspenseMonitor = (asOf?: string) =>
  useQuery({
    queryKey: key('suspense', asOf ?? 'today'),
    queryFn: () =>
      api.get<SuspenseMonitor>('/controls/suspense', { query: asOf ? { asOf } : undefined }),
  });
