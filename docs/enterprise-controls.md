...
Accounts of subtype `SUSPENSE` and the `SUSPENSE`, `FIXED_ASSET_CLEARING` and
`GOODS_RECEIVED_NOT_INVOICED` mappings (seed: 1900 Suspense / Clearing, 1590
Fixed Asset Clearing, 2160 GRNI) are expected to return to zero.
`SuspenseService.monitor(companyId, asOf)` (`modules/accounting/suspense`, the
single implementation shared with the accounting core) walks each account's
posted lines in date order and reports:
...
It feeds `GET /accounting/suspense` (page **Accounting → Suspense Accounts**,
with the owner per account and a "Clear" deep link to a reclassification
journal), the integrity check `SUSPENSE_BALANCE`
...
- Department (dimension) specific approval rules are not implemented; branch
  scope is.
- Suspense accounts carry a responsible person (`owner_user_id`); no department-level ownership yet.
