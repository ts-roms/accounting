...
| GET / PUT             | `/consolidation/groups/:id/account-mappings?companyId` (mapped, unmatched, targets), `PUT { companyId, mappings[{ accountId, groupAccountCode, notes? }] }` replaces one member's group chart mappings  | view / manage                                 |
...
| POST                        | `/treasury/petty-cash/vouchers/:id/submit` (opens the approval workflow) / `approve` / `post` / `void { reason, voidDate? }`                           | `petty-cash.approve` (delegable) / `petty-cash.post`                            |
...
| POST                        | `/bank-statements { bankAccountId, statementDate, openingBalance, closingBalance, lines[] }`, `/bank-statements/parse` (multipart `file`: MT940 / camt.053 / OFX -> import shape + warnings, nothing stored) | `bank-statement.import`                         |
...
| GET / POST / PATCH  | `/approval-workflows`, `/approval-workflows/:id` `{ documentType, name, description?, minAmount, maxAmount?, priority, allowSelfApproval, steps[{ name, requiredPermission, minApprovers, approverUserIds?, approverRoleIds? }] }` (PATCH also `status`) | `approval.view` / `workflow.manage`                 |
| GET                 | `/approval-workflows/matrix` (every document type -> amount bands -> steps with resolved approvers), `/approval-workflows/approver-options` (users + roles an editor may name)                                                                           | `approval.view` / `workflow.manage`                 |
...
| Method      | Path                                                                                                                                 | Permission                                                         |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------ |
| GET         | `/ai/status` -> `{ provider, model, ocr: 'TESSERACT' \| null, advisoryOnly: true }`                                                  | `ai.view`                                                          |
